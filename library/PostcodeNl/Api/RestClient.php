...
class PostcodeNl_Api_RestClient
{
     ...
     * @return PostcodeNl_Model_Address
     * @throws PostcodeNl_Api_RestClient_InputInvalidException
     *
    ...
    {
        $postcode = $this->_cleanupPostcode($postcode);
        ...
        return new PostcodeNl_Model_Address($response['data']);
    }
     ...
     * street - (string) Street name in accordance with "BAG (Basisregistraties adressen en gebouwen)". In capital and lowercase letters, including punctuation marks and accents. This field is at most 80 characters in length. Filled with "Postbus" in case it is a range of PO boxes.
     * streetNen - (string) Street name in NEN-5825 notation, which has a lower maximum length. In capital and lowercase letters, including punctuation marks and accents. This field is at most 24 characters in length. Filled with "Postbus" in case it is a range of PO boxes.
     * startHouseNumber - (int) First house number or first PO box number of the range.
     * endHouseNumber - (int) Last house number or last PO box number of the range.
     * houseNumberType - (string) The type of house numbers in the range: odd or even
     * city - (string) Official city name in accordance with "BAG (Basisregistraties adressen en gebouwen)". In capital and lowercase letters, including punctuation marks and accents. This field is at most 80 characters in length.
     * cityShort - (string) City name, shortened to fit a lower maximum length. In capital and lowercase letters, including punctuation marks and accents. This field is at most 24 characters in length.
     * municipality - (string) Municipality name in accordance with "BAG (Basisregistraties adressen en gebouwen)". In capital and lowercase letters, including punctuation marks and accents. This field is at most 80 characters in length. Examples: "Baarle-Nassau", "'s-Gravenhage", "Haarlemmerliede en Spaarnwoude".
     * municipalityShort - (string) Municipality name, shortened to fit a lower maximum length. In capital and lowercase letters, including punctuation marks and accents. This field is at most 24 characters in length. Examples: "Baarle-Nassau", "'s-Gravenhage", "Haarlemmerliede c.a.".
     * province - (string) Official name of the province, correctly cased and with dashes where applicable.
     */
    public function lookupRange($postcode) {
        $postcode = $this->_cleanupPostcode($postcode);
        // Test postcode format
        if (!$this->isValidPostcodeFormat($postcode)) {
            throw new PostcodeNl_Api_RestClient_InputInvalidException('Postcode `' . $postcode . '` needs to be in the 1234AB format.');
        ...
        $ranges = array();
        require_once __DIR__.'/../Models/PostcodeRange.php';
        foreach ($response['data'] as $range) {
            $ranges[] = new PostcodeNl_Model_PostcodeRange($range);
        }
        // Successful response!
        return $ranges;
    }
     ...
     * @param bool $validateHouseNumberAddition
     * @return PostcodeNl_Model_Address
     * @throws PostcodeNl_Api_RestClient_InputInvalidException
     *
     * @see https://api.postcode.nl/documentation
     ...
     * houseNumberAdditions - (array) List of all house number additions having the postcode and houseNumber which was input.
     */
    public function lookupExactAddress($city, $street, $houseNumber, $houseNumberAddition = '', $validateHouseNumberAddition = false) {
        $houseNumber = trim($houseNumber);
        $houseNumberAddition = trim($houseNumberAddition);
        ...
        // Use the regular validation function
        $url = $this->_restApiUrl .'/addresses/exact/' . rawurlencode($city) . '/' . rawurlencode($street) . '/' . rawurlencode($houseNumber) . '/' . rawurlencode($houseNumberAddition);
     ...
     * houseNumberAdditions - (array) List of all house number additions having the postcode and houseNumber which was input.
     */
    public function lookupRd($rdX, $rdY) {
        // Test rdx format
        if (!ctype_digit($rdX)) {
            throw new PostcodeNl_Api_RestClient_InputInvalidException('RdX number `' . $rdX . '` must contain digits only.');
        } elseif ($rdX < 0 || $rdX > 300000) {
            throw new PostcodeNl_Api_RestClient_InputInvalidException('RdX number `' . $rdX . '` must be between 0 and 300000 meters.');
        }
        // Test rdy format
        if (!ctype_digit($rdY)) {
            throw new PostcodeNl_Api_RestClient_InputInvalidException('RdY number `' . $rdY . '` must contain digits only.');
        } elseif ($rdY < 300000 || $rdY > 620000) {
            throw new PostcodeNl_Api_RestClient_InputInvalidException('RdY number `' . $rdY . '` must be between 300000 and 620000 meters.');
        }
        // Use the regular validation function
        $url = $this->_restApiUrl .'/addresses/rd/' . rawurlencode($rdX) . '/' . rawurlencode($rdY);
        ...
        require_once __DIR__.'/../Models/Address.php';
        // Successful response!
        return array('address' => new PostcodeNl_Model_Address($response['data']['address']), 'distance' => $response['data']['distance']);
    }
     ...
     * houseNumberAdditions - (array) List of all house number additions having the postcode and houseNumber which was input.
     */
    public function lookupLatLon($lat, $lon) {
        // Test rdx format
        if (!is_numeric($lat)) {
            throw new PostcodeNl_Api_RestClient_InputInvalidException('Latitude `' . $lat . '` must contain digits only.');
        } elseif ($lat < 50.75 || $lat > 53.51) {
            throw new PostcodeNl_Api_RestClient_InputInvalidException('Latitude `' . $lat . '` must be between 50.75 and 53.51 degrees.');
        }
        // Test rdy format
        if (!is_numeric($lon)) {
            throw new PostcodeNl_Api_RestClient_InputInvalidException('Longitude `' . $lon . '` must contain digits only.');
        } elseif ($lon < 3.35 || $lon > 7.23) {
            throw new PostcodeNl_Api_RestClient_InputInvalidException('Longitude `' . $rdY . '` must be between 3.35 and 7.23 degrees.');
        }
        // Use the regular validation function
        $url = $this->_restApiUrl .'/addresses/latlon/' . rawurlencode($lat) . '/' . rawurlencode($lon);
     ...
     * @return string
     */
    private function _cleanupPostcode($postcode) {
        // Remove spaces in postcode ('1234 AB' should be '1234AB')
